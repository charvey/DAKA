package daka.compute.fpgrowth;

import daka.compute.helpers.Pair;
import daka.compute.helpers.PairComparator;
import daka.compute.helpers.SequenceFileIterable;
import daka.compute.helpers.StatusUpdater;
import daka.compute.helpers.TopKPatternsOutputConverter;
import daka.compute.helpers.TransactionIterator;
...
public class FPGrowth<A extends Comparable<? super A>>
{
			...
			Iterator<Pair<List<A>, Long>> transactions, int minSupport)
	{
		...
		Collections.sort(fList, new PairComparator<A, Long>());
	 ...
	 */
	private Map<Integer, FPHeap> fpGrowth(FPTree tree, long minSupportValue,
			int k, Collection<Integer> requiredFeatures,
			TopKPatternsOutputConverter<A> outputCollector,
			StatusUpdater updater) throws IOException
	{

		Map<Integer, FPHeap> patterns = Maps.newHashMap();
		...
		for (int i = tree.getHeaderTableCount() - 1; i >= 0; i--)
		{
			...
			if (requiredFeatures.contains(attribute))
			{
				...
				FPHeap frequentPatterns = growth(tree, minSupport, k,
	...
	private static FPHeap generateSinglePathPatterns(FPTree tree, int k,
			long minSupport)
	{
		FPHeap frequentPatterns = new FPHeap(k, false);
	...
	private static FPHeap growth(FPTree tree, MutableLong minSupportMutable,
			int k, FPTreeDepthCache treeCache, int level, int currentAttribute,
			StatusUpdater updater)
	{

		FPHeap frequentPatterns = new FPHeap(k, true);
	...
	private static FPHeap growthBottomUp(FPTree tree,
	...
	private static FPHeap growthTopDown(FPTree tree,
			MutableLong minSupportMutable, int k, FPTreeDepthCache treeCache,
			int level, boolean conditionalOfCurrentAttribute,
			int currentAttribute, StatusUpdater updater)
	{
	...
	private static FPHeap mergeHeap(FPHeap frequentPatterns,
			FPHeap returnedPatterns, int attribute, long count,
	...
	private static void pruneFPTree(long minSupport, FPTree tree)
	{
		...
		for (int i = 0; i < tree.getHeaderTableCount(); i++)
		{
			...
			Map<Integer, Integer> prevNode = Maps.newHashMap();
			int justPrevNode = -1;
			while (nextNode != -1)
			{
