...
public final class TopKStringPatterns implements Writable
{
	...
	public TopKStringPatterns merge(TopKStringPatterns pattern, int heapSize)
	{
		...
		Iterator<Pair<List<String>, Long>> myIterator = frequentPatterns
				.iterator();
		...
		for (int i = 0; i < heapSize; i++)
		{
			...
			if (myItem != null && otherItem != null)
			{
				...
				if (cmp == 0)
				{
					cmp = myItem.getFirst().size()
							- otherItem.getFirst().size();
					if (cmp == 0)
					{
						for (int j = 0; j < myItem.getFirst().size(); j++)
						{
							cmp = myItem.getFirst().get(j)
									.compareTo(otherItem.getFirst().get(j));
							if (cmp != 0)
							{
								break;
							}
