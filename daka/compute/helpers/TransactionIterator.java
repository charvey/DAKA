 ...
 */
public class TransactionIterator<T> extends
		ForwardingIterator<Pair<int[], Long>>
{
			...
			final Map<T, Integer> attributeIdMapping)
	{
		...
		delegate = Iterators.transform(transactions,
				new Function<Pair<List<T>, Long>, Pair<int[], Long>>()
				{
					...
					public Pair<int[], Long> apply(Pair<List<T>, Long> from)
					{
						...
						for (T attribute : from.getFirst())
						{
							...
							{
								transactionBuffer[index++] = attributeIdMapping
										.get(attribute);
							}
						}
						int[] transactionList = new int[index];
						System.arraycopy(transactionBuffer, 0, transactionList,
								0, index);
						return new Pair<int[], Long>(transactionList, from
								.getSecond());
					}
				});
	}
