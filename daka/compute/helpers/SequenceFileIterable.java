		...
		implements Iterable<Pair<K, V>>
{
	...
	/**
	 * Like {@link #SequenceFileIterable(Path, boolean, Configuration)} but key
	 * and value instances are not reused by default.
	 * 
	 * @param path
	 ...
	 *            if true, reuses instances of the key and value object instead
	 *            of creating a new one for each read from the file
	 */
	public SequenceFileIterable(Path path, boolean reuseKeyValueInstances,
			Configuration conf)
	{
		this.path = path;
